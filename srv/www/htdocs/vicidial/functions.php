...
function horizontal_bar_chart($campaign_id, $days_graph, $title, $link, $metric, $metric_name, $more_link, $END_DATE, $download_link, $chart_title)
{
	...
	if ($max_count < 1) {
		...
		if (strlen($chart_title) < 4) {
			$chart_title = "$days_graph Day $out_in_type $metric_name for this $title";
		}
		if ($more_link > 0) {
	...
	if ($SSenable_languages == '1') {
		if ($SSlanguage_method != 'DISABLED') {
			if ((strlen($VUselected_language) > 0) and ($VUselected_language != 'default English')) {
				if ($English_text == "Y" && !preg_match('/english/i', $VUselected_language)) {
					$English_text = "YES";
				}
				if ($English_text == "N" && !preg_match('/english/i', $VUselected_language)) {
					$English_text = "NO";
				}
...
function TimeToText($interval)
{
	...
	if ($minutes > 0) {
		if ($minutes == 1) {
			$return_str .= "$minutes " . _QXZ("minute") . ", ";
		} else {
			$return_str .= "$minutes " . _QXZ("minutes") . ", ";
		}
	}
	if ($seconds > 0) {
		if ($seconds == 1) {
			$return_str .= "$seconds " . _QXZ("second");
		} else {
			$return_str .= "$seconds " . _QXZ("seconds");
		}
	}
			...
			if (strlen($dial_statuses) > 2) {
				...
				if (strlen($Gct_holidays) > 2) {
					...
					if ($sthCrows > 0) {
						...
						if ($DB) {
							$dialable_output .= "CALL TIME HOLIDAY FOUND!   $local_call_time|$holiday_id|$holiday_date|$holiday_name|$Gct_default_start|$Gct_default_stop|\n";
						}
					}
				}
					...
					if (strlen($state_rules[$b]) > 1) {
						...
						if ((strlen($Sct_holidays) > 2) or ((strlen($holiday_id) > 2) and (strlen($Sholiday_id) < 2))) {
							...
							if ($sthCrows > 0) {
								...
								if ($DB) {
									$dialable_output .= "$holidaytype   |$Gstate_call_time_id|$Gstate_call_time_state|$Sholiday_id|$Sholiday_date|$Sholiday_name|$Gsct_default_start|$Gsct_default_stop|\n";
								}
							}
						}
						$r = 0;
				...
				if ($DB) {
					$dialable_output .= "$camplists_ct|$stmt\n";
				}
				$k = 0;
				$camp_lists = '';
					...
					if ($cur_call_time != "campaign") {
						...
						if (strlen($Gct_holidays) > 2) {
							...
							if ($sthCrows > 0) {
								...
								if ($DB) {
									$dialable_output .= "LIST CALL TIME HOLIDAY FOUND!   $local_call_time|$holiday_id|$holiday_date|$holiday_name|$Gct_default_start|$Gct_default_stop|\n";
								}
							}
						}
						### END Check for outbound holiday ###
