...
} elseif (isset($_POST["CBstatus"])) {
	$CBstatus = $_POST["CBstatus"];
}
if (isset($_GET["archive_search"])) {
	$archive_search = $_GET["archive_search"];
} elseif (isset($_POST["archive_search"])) {
	$archive_search = $_POST["archive_search"];
}
if (isset($_GET["archive_log"])) {
	$archive_log = $_GET["archive_log"];
} elseif (isset($_POST["archive_log"])) {
	$archive_log = $_POST["archive_log"];
}
...
if (isset($_GET["gdpr_action"])) {
	$gdpr_action = $_GET["gdpr_action"];
} elseif (isset($_POST["gdpr_action"])) {
	$gdpr_action = $_POST["gdpr_action"];
}
if (isset($_GET["CIDdisplay"])) {
	$CIDdisplay = $_GET["CIDdisplay"];
} elseif (isset($_POST["CIDdisplay"])) {
	$CIDdisplay = $_POST["CIDdisplay"];
}
if (isset($_GET["modify_log_submit"])) {
	$modify_log_submit = $_GET["modify_log_submit"];
} elseif (isset($_POST["modify_log_submit"])) {
	$modify_log_submit = $_POST["modify_log_submit"];
}
if (isset($_GET["modify_log_table"])) {
	$modify_log_table = $_GET["modify_log_table"];
} elseif (isset($_POST["modify_log_table"])) {
	$modify_log_table = $_POST["modify_log_table"];
}
if (isset($_GET["modify_log_status"])) {
	$modify_log_status = $_GET["modify_log_status"];
} elseif (isset($_POST["modify_log_status"])) {
	$modify_log_status = $_POST["modify_log_status"];
}
if (isset($_GET["modify_old_status"])) {
	$modify_old_status = $_GET["modify_old_status"];
} elseif (isset($_POST["modify_old_status"])) {
	$modify_old_status = $_POST["modify_old_status"];
}
if (isset($_GET["vicidial_id"])) {
	$vicidial_id = $_GET["vicidial_id"];
} elseif (isset($_POST["vicidial_id"])) {
	$vicidial_id = $_POST["vicidial_id"];
}
...
echo 'BEGIN modify log status functions';


echo '----------modify_log_submit------------';
print_r($modify_log_submit);
echo '----------------------';
...
echo '---------modify_log_table-------------';
print_r($modify_log_table);


echo '---------modify_log_status-------------';
print_r($modify_log_status);

echo '---------modify_old_status-------------';
print_r($modify_old_status);
...
if ($enable_gdpr_download_deletion > 0) {
	...
	if ($gdpr_display >= 1 && $gdpr_action && $lead_id) {
		...
		if ($gdpr_action == "confirm_purge") {
			...
			if ($archive_table_name != "vicidial_list") {
				$mysql_stmt .= " UNION (select list_id from " . $archive_table_name . " where lead_id='$lead_id')";
			}
		...
		for ($t = 0; $t < $table_count; $t++) {
			...
			if ($archive_table_name != $table_name) {
				$mysql_stmt .= " UNION (select * from " . $archive_table_name . " where lead_id='$lead_id')";
			}
			$list_id = "";
			$HTML_header = "";
			$CSV_header = "";
...
if (strlen($row[19]) > 0) {
	$label_lead_id =			$row[19];
}
if (strlen($row[20]) > 0) {
	$label_list_id =			$row[20];
}
if (strlen($row[21]) > 0) {
	$label_entry_date =		$row[21];
}
if (strlen($row[22]) > 0) {
	$label_gmt_offset_now =	$row[22];
}
if (strlen($row[23]) > 0) {
	$label_source_id =			$row[23];
}
if (strlen($row[24]) > 0) {
	$label_called_since_last_reset = $row[24];
}
if (strlen($row[25]) > 0) {
	$label_status =			$row[25];
}
...
if (strlen($row[27]) > 0) {
	$label_date_of_birth =		$row[27];
}
if (strlen($row[28]) > 0) {
	$label_country_code =		$row[28];
}
if (strlen($row[29]) > 0) {
	$label_last_local_call_time =	$row[29];
}
if (strlen($row[30]) > 0) {
	$label_called_count =		$row[30];
}
if (strlen($row[31]) > 0) {
	$label_rank =				$row[31];
}
...
<script language="JavaScript">
	...
	if ($end_call > 0) {
		...
		$stmt = "SELECT uniqueid,lead_id,list_id,campaign_id,call_date,start_epoch,end_epoch,length_in_sec,status,phone_code,phone_number,user,comments,processed,user_group,term_reason,alt_dial from vicidial_log where lead_id='" . mysqli_real_escape_string($link, $lead_id) . "' order by call_date desc limit 500;";
		$rslt = mysql_to_mysqli($stmt, $link);
		...
		while ($Alogs_to_print > $y) {
			...
			if (($LOGmodify_leads == '3') or ($LOGmodify_leads == '4')) {
				...
				$agent_log .= "<td align=center><font size=2> $row[14]</td>\n";
			}
			$agent_log .= "<td align=right><font size=2> &nbsp; $row[15] </td>\n";
			$agent_log .= "<td align=right><font size=2> &nbsp; $row[17] </td>\n";

			$agent_log .= "<td align=right><font size=2> &nbsp; $row[18] </td></tr>\n";
		...
		if ($lead_count > 0) {
			...
			if ($LOGadmin_hide_phone_data != '0') {
				if ($DB > 0) {
					echo "HIDEPHONEDATA|$row[11]|$LOGadmin_hide_phone_data|\n";
				}
				$phone_temp = $row[11];
			...
			if ($LOGadmin_hide_lead_data != '0') {
				if ($DB > 0) {
					echo "HIDELEADDATA|$row[5]|$row[6]|$row[12]|$row[13]|$row[14]|$row[15]|$row[16]|$row[17]|$row[18]|$row[19]|$row[20]|$row[21]|$row[22]|$row[26]|$row[27]|$row[28]|$LOGadmin_hide_lead_data|\n";
				}
				if (strlen($row[5]) > 0) {
					$data_temp = $row[5];
					$row[5] = preg_replace("/./", 'X', $data_temp);
				}
				if (strlen($row[6]) > 0) {
					$data_temp = $row[6];
		...
		if ($lead_id != 'NEW') {
			...
			if ($allow_emails > 0) {
				...
				while ($logs_to_print > $u) {
					...
					if (mb_strlen($row[6], 'utf-8') > 100) {
						$row[6] = mb_substr($row[6], 0, 100, 'utf-8') . "...";
					}
