...
testEcho($completed_statuses, '$completed_statuses__1');
...
testEcho($completed_statuses, '$completed_statuses__2');
...
testEcho($completed_statuses, '$completed_statuses__3');
...
if (strlen($group[0]) < 1) {
	...
	testEcho($lists_id_str, '$lists_id_str');
	testEcho($stmt, '$stmt__1');
	...
	testEcho($list_id_SQL, '$list_id_SQL');
	...
	while ($i < $listids_to_print) {
		...
		if ($list_name_to_print > 0) {
			...
			if ($row[1] == 'Y') {
				$LISTIDlist_active[$i] = _QXZ("ACTIVE", 8);
				$graph_stats[$i][1] .= " (" . _QXZ("ACTIVE") . ")";
			} else {
				$LISTIDlist_active[$i] = _QXZ("INACTIVE", 8);
				$graph_stats[$i][1] .= " (" . _QXZ("INACTIVE") . ")";
			}
		}
	...
	$OUToutput .= "\n";
	$OUToutput .= "\n";
	$OUToutput .= "---------- " . _QXZ("STATUS FLAGS SUMMARY123:", 24) . " (" . _QXZ("and % of leads in selected lists") . ")     <a href=\"$PHP_SELF?DB=$DB$groupQS&SUBMIT=$SUBMIT&file_download=2&search_archived_data=$search_archived_data\">" . _QXZ("DOWNLOAD") . "</a>\n";
	$OUToutput .= "+------------------+------------+----------+\n";
	...
	$OUToutput .= "| " . _QXZ("Completed123", 16) . " | $COMP_count |  $COMP_percent% |\n";
	$OUToutput .= "+------------------+------------+----------+\n";
	$OUToutput .= "\n";
	...
	while ($i < $listids_to_print) {
		...
		$header_list_id =	sprintf("%-51s", $header_list_id);
		while (strlen($header_list_id) > 51) {
			$header_list_id = substr("$header_list_id", 0, -1);
		}
		$header_list_count =	sprintf("%10s", $LISTIDcalls[$i]);
		while (strlen($header_list_count) > 10) {
			$header_list_count = substr("$header_list_count", 0, -1);
		}
		...
		if ($SALE_count > $max_flags) {
			$max_flags = $SALE_count;
		}
		...
		if ($COMP_count > $max_flags) {
			$max_flags = $COMP_count;
		}
		...
		$HA_count =	sprintf("%9s", "$HA_count");
		while (strlen($HA_count) > 9) {
			$HA_count = substr("$HA_count", 0, -1);
		}
		$SALE_count =	sprintf("%9s", "$SALE_count");
		while (strlen($SALE_count) > 9) {
			$SALE_count = substr("$SALE_count", 0, -1);
		}
		$DNC_count =	sprintf("%9s", "$DNC_count");
		while (strlen($DNC_count) > 9) {
			$DNC_count = substr("$DNC_count", 0, -1);
		}
		$CC_count =	sprintf("%9s", "$CC_count");
		while (strlen($CC_count) > 9) {
			$CC_count = substr("$CC_count", 0, -1);
		}
		$NI_count =	sprintf("%9s", "$NI_count");
		while (strlen($NI_count) > 9) {
			$NI_count = substr("$NI_count", 0, -1);
		}
		$UW_count =	sprintf("%9s", "$UW_count");
		while (strlen($UW_count) > 9) {
			$UW_count = substr("$UW_count", 0, -1);
		}
		$SC_count =	sprintf("%9s", "$SC_count");
		while (strlen($SC_count) > 9) {
			$SC_count = substr("$SC_count", 0, -1);
		}
		$COMP_count =	sprintf("%9s", "$COMP_count");
		while (strlen($COMP_count) > 9) {
			$COMP_count = substr("$COMP_count", 0, -1);
		}
		...
		while ($r < $liststatussum_to_print) {
			...
			if ($DB) {
				$MAIN .= "$r|$LISTIDstatus[$r]|$LISTIDcounts[$r]|    |$row[0]|$row[1]|<BR>\n";
			}
			$r++;
		}
		...
		while ($r < $liststatussum_to_print) {
			...
			$LISTID_status_count =	sprintf("%10s", $LISTIDcounts[$r]);
			while (strlen($LISTID_status_count) > 10) {
				$LISTID_status_count = substr("$LISTID_status_count", 0, -1);
			}
			$LISTIDname =	sprintf("%-42s", "$LIDstatus_format | $statname_list[$LIDstatus]");
			while (strlen($LISTIDname) > 42) {
				$LISTIDname = substr("$LISTIDname", 0, -1);
			}
