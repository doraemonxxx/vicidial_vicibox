...
if (($submit != "submit") && ($confirm != "CONFIRM")) {
	...
	$lists_stmt = "SELECT list_id, list_name, campaign_id FROM vicidial_lists WHERE campaign_id IN ($allowed_campaigns_sql) $activeSQL ORDER BY list_id;";
	if ($DB) {
		echo "|$lists_stmt|\n";
	}

	// testEcho($lists_stmt, '$lists_stmt_____');
	...
	while ($allowed_lists_count < $num_rows) {
		...
		### start new
		$list_id_ary_str .= "'$lists_row[0]',";
		$list_name_ary_str .= "'$lists_row[1]',";
		$campaign_id_ary_str .= "'$lists_row[2]',";
		### end new
	...
	// testEcho($list_id_ary_str, '$list_id_ary_str_____');
	// testEcho($list_name_ary_str, '$list_name_ary_str____');
	// testEcho($campaign_id_ary_str, '$campaign_id_ary_str_____');
	// testEcho($list_lead_count_ary, '$list_lead_count_ary______');
	### end new
?>

	<script language="JavaScript">
		console.info('Copyright @2022 TRUALLIANT BPO INC. All Rights Reserved.');

		<?= $list_id_ary_str ?>
		<?= $list_name_ary_str ?>
		<?= $campaign_id_ary_str ?>
		<?= $count = "var list_lead_count_ary=" . json_encode($list_lead_count_ary) . ";\n" ?>
		<?= $allowed_lists_count_js = "var allowed_lists_count=" . $allowed_lists_count . ";\n" ?>


		function FilterLists(FromBox) {

			// set destination_list_id value and options to default - always START
			document.getElementById("destination_list_id").value = "";
			document.getElementById("destination_list_id").options.length = 1;
			// set destination_list_id value and options to default - always END

			console.info('-------FilterLists-------');
			if (!FromBox) {
				alert("NO");
				return false;
			}
			...
			console.info('selected Camps: ', selectedCampaigns)

			function listText(index) {
				return list_id_ary[index] + " - " + list_name_ary[index] + " (" + list_lead_count_ary[index] + " " + 'leads' + ")";
			}

			function listValue(index) {
				return list_id_ary[index];
			}

			// Clear List menu
			document.getElementById('available_lists').options.length = 0;
			var new_list = new Option();
			new_list.value = "";
			new_list.text = "-- SELECT LISTS BELOW --";
			document.getElementById('available_lists')[0] = new_list;
			list_id_index = 1;

			function setOptionsToSelectLists(array, useCampMatch = true) {

				function makeOptions(index) {
					var new_list = new Option();
					new_list.value = listValue(index);
					new_list.text = listText(index);
					document.getElementById('available_lists')[list_id_index] = new_list;
					list_id_index++;
				}

				for (j = 0; j < array.length; j++) {
					if (useCampMatch) {
						var campaignID = "/\|" + array[j] + "\|/g";
			...
			if (selectedCampaigns == '||') {
				setOptionsToSelectLists(campaign_id_ary, false);
				console.info('set to default lists');
			} else {
				setOptionsToSelectLists(campaign_id_ary, true);
				console.info('filter the lists using camps')
			}
		}
	</script>
...
<?php
	...
	### start new
	########### campaign selection list sql start
	...
	// testEcho($groups, '$groups______');
	// testEcho($group_names, '$group_names____');
	########### campaign selection list sql END


	########### JS HTML EVENT FOR camp select list start
	if (preg_match('/MSIE/i', $_SERVER['HTTP_USER_AGENT'])) {
		$JS_events = "onBlur='FilterLists(this.form.group)' onKeyUp='FilterLists(this.form.group)'";
	} else {
		$JS_events = "onMouseUp='FilterLists(this.form.group)' onBlur='FilterLists(this.form.group)' onKeyUp='FilterLists(this.form.group)'";
	}

	########### JS HTML EVENT FOR camp select list end


	########### Campaigns Lists start
	echo "<tr bgcolor=#$SSstd_row2_background><td align=right>" . _QXZ("Campaigns") . "</td><td align=left>\n";
	$style = 'style="resize: auto;"';
	echo "<select $JS_events $style size=" . ($campaigns_to_print < 10 ? 10 : $campaigns_to_print + 10) . " name=group[] id='group' multiple>\n";
	echo "<option value>-- " . _QXZ("SELECT CAMPAIGNS BELOW") . " --</option>\n";
	$i = 0;
	while ($campaigns_to_print > $i) {
		if (preg_match("/$groups[$i]\|/i", $group_string)) {
			echo "<option selected value=\"$groups[$i]\">$groups[$i] - $group_names[$i]</option>\n";
		} else {
			echo "<option value=\"$groups[$i]\">$groups[$i] - $group_names[$i]</option>\n";
		}
		$i++;
	}
	echo "</select>";
	...
	###### modification for avaialble lists start ######
	# Available Lists
	echo "<tr bgcolor=#$SSstd_row2_background><td align=right>" . _QXZ("Available Lists") . "</td><td align=left>\n";
	echo "<select $style size=" . ($num_rows < 10 ? 10 : $num_rows + 10) . " name=available_lists[] id='available_lists' multiple onClick='PopulateMergeMenu()'>\n";
	...
	###### modification for avaialble lists end ######
