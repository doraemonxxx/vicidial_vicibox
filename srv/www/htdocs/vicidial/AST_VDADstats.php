...
if (isset($_GET["print_calls"])) {
	$print_calls = $_GET["print_calls"];
} elseif (isset($_POST["print_calls"])) {
	$print_calls = $_POST["print_calls"];
}
if (isset($_GET["outbound_rate"])) {
	$outbound_rate = $_GET["outbound_rate"];
} elseif (isset($_POST["outbound_rate"])) {
	$outbound_rate = $_POST["outbound_rate"];
}
if (isset($_GET["costformat"])) {
	$costformat = $_GET["costformat"];
} elseif (isset($_POST["costformat"])) {
	$costformat = $_POST["costformat"];
}
if (isset($_GET["include_rollover"])) {
	$include_rollover = $_GET["include_rollover"];
} elseif (isset($_POST["include_rollover"])) {
	$include_rollover = $_POST["include_rollover"];
}
if (isset($_GET["carrier_stats"])) {
	$carrier_stats = $_GET["carrier_stats"];
} elseif (isset($_POST["carrier_stats"])) {
	$carrier_stats = $_POST["carrier_stats"];
}
if (isset($_GET["bottom_graph"])) {
	$bottom_graph = $_GET["bottom_graph"];
} elseif (isset($_POST["bottom_graph"])) {
	$bottom_graph = $_POST["bottom_graph"];
}
if (isset($_GET["agent_hours"])) {
	$agent_hours = $_GET["agent_hours"];
} elseif (isset($_POST["agent_hours"])) {
	$agent_hours = $_POST["agent_hours"];
}
if (isset($_GET["group"])) {
	$group = $_GET["group"];
...
if ($list_rows > 0) {
	...
	testEcho(mysqli_fetch_row($list_rslt), 'mysqli_fetch_row($list_rslt)______');
	...
	while ($list_row = mysqli_fetch_row($list_rslt)) {
		...
		testEcho($list_row[0], '$list_row[0]_____');
	...
	testEcho($list_id_ary_str, '$list_id_ary_str____');
	testEcho($list_name_ary_str, '$list_name_ary_str_____');
	...
	testEcho($list_id_ary, '$list_id_ary');
}
$list_options .= "</select>\n";
...
$HEADER_b .= "	console.log('DIPOTA NGA CODE MANGO2x');\n";
...
$HEADER_b .= "	console.log('selectedCampaigns____', selectedCampaigns);\n";
...
$HEADER_b .= "	console.log('campaign_matches____', campaign_matches);\n";
...
testEcho($campaigns_to_print, '$campaigns_to_print__');
...
if (strlen($group[0]) < 1) {
	...
	if ($DB) {
		$ASCII_text .= "$AGENTsec|$Ctally_to_print|$stmt\n";
	}
	...
	while ($i < $statuses_to_print) {
		...
		if ($non_latin < 1) {
			...
			while (strlen($status_name) > 20) {
				$status_name = substr("$status_name", 0, -1);
			}
			$statcat =	sprintf("%-20s", $statcat_list[$RAWstatus]);
			while (strlen($statcat) > 20) {
				$statcat = substr("$statcat", 0, -1);
			}
		} else {
	...
	$i = 0;
	while ($i < $users_to_print) {
		$user =	sprintf("%-6s", $RAWuser[$i]);
		while (strlen($user) > 6) {
			$user = substr("$user", 0, -1);
		}
		if ($non_latin < 1) {
			$full_name =	sprintf("%-15s", $RAWfull_name[$i]);
