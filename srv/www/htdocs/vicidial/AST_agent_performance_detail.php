...
if (isset($_GET["live_agents"])) {
	$live_agents = $_GET["live_agents"];
} elseif (isset($_POST["live_agents"])) {
	$live_agents = $_POST["live_agents"];
}
...
if (isset($_GET["time_in_sec"])) {
	$time_in_sec = $_GET["time_in_sec"];
} elseif (isset($_POST["time_in_sec"])) {
	$time_in_sec = $_POST["time_in_sec"];
}
if (isset($_GET["breakdown_by_date"])) {
	$breakdown_by_date = $_GET["breakdown_by_date"];
} elseif (isset($_POST["breakdown_by_date"])) {
	$breakdown_by_date = $_POST["breakdown_by_date"];
}
...
if (isset($_GET["show_defunct_users"])) {
	$show_defunct_users = $_GET["show_defunct_users"];
} elseif (isset($_POST["show_defunct_users"])) {
	$show_defunct_users = $_POST["show_defunct_users"];
}
...
if ($DB) {
	$HTML_text .= "$user_group_string|$user_group_ct|$user_groupQS|$i<BR>";
}
...
while ($user_groups_to_print > $o) {
	if (preg_match("/$allowed_user_groups[$o]\|/i", $user_group_string)) {
		$HTML_text .= "<option selected value=\"$allowed_user_groups[$o]\">$allowed_user_groups[$o]</option>\n";
	} else {
		$HTML_text .= "<option value=\"$allowed_user_groups[$o]\">$allowed_user_groups[$o]</option>\n";
	}
	$o++;
}
$HTML_text .= "</SELECT>\n";
...
while ($users_to_print > $o) {
	if (preg_match("/$user_list[$o]\|/i", $users_string)) {
		$HTML_text .= "<option selected value=\"$user_list[$o]\">$user_list[$o] - $user_names[$o]</option>\n";
	} else {
		$HTML_text .= "<option value=\"$user_list[$o]\">$user_list[$o] - $user_names[$o]</option>\n";
	}
	$o++;
}
$HTML_text .= "</SELECT>\n";
...
if (!$group) {
	...
	while ($stat_row = mysqli_fetch_row($stat_rslt)) {
		...
		while ($i < $stat_rows_to_print) {
			...
			if ($DB) {
				$HTML_text .= "$full_name_val - $user_group_val\n";
			}
			// the calls
			$calls[$q] = $row[0];
			$talk_sec[$q] = $row[1];
	...
	while ($m < $k) {
		...
		while ($n < $j) {
			...
			while ($i < $rows_to_print) {
				if (($Suser == "$user[$i]") and ($Sstatus == "$status[$i]")) {
					// this is the call that will add new value to the calls that is not the same in the database
					// testEcho($Scalls, '$Scalls____');
					testEcho(($Scalls + $calls[$i]), '($Scalls + $calls[$i])');
		...
		if ($non_latin < 1) {
			...
			while (strlen($Suser_group) > 20) {
				$Suser_group = substr("$Suser_group", 0, -1);
			}
			$Slast_user_group = sprintf("%-20s", $Slast_user_group);
			while (strlen($Slast_user_group) > 20) {
				$Slast_user_group = substr("$Slast_user_group", 0, -1);
			}
		...
		# the time column and value in the table with check Show %s
		$pfUSERtime_MS = sec_convert($Stime, $TIME_H_agentperfdetail);
		$pfUSERtotTALK_MS = sec_convert($Stalk_sec, $TIME_H_agentperfdetail);
		# the talk percentage
		testEcho($Stalk_sec, '$Stalk_sec______');
		$pfUSERtotTALK_MS_pct = sprintf("%0.2f", MathZDC(100 * $Stalk_sec, $Stime));
		$pfUSERavgTALK_MS = sec_convert($Stalk_avg, $TIME_M_agentperfdetail);

		# the pause = H
		$USERtotPAUSE_MS = sec_convert($Spause_sec, $TIME_H_agentperfdetail);
		# the pause percentage
		$pfUSERtotPAUSE_MS_pct = sprintf("%0.2f", MathZDC(100 * $Spause_sec, $Stime));
		# the pause avg = M
		$USERavgPAUSE_MS = sec_convert($Spause_avg, $TIME_M_agentperfdetail);
		...
		$USERavgWAIT_MS = sec_convert($Swait_avg, $TIME_M_agentperfdetail);
		$USERtotDISPO_MS = sec_convert($Sdispo_sec, $TIME_H_agentperfdetail);
		# the dispo percentage
		$pfUSERtotDISPO_MS_pct = sprintf("%0.2f", MathZDC(100 * $Sdispo_sec, $Stime));
		$USERavgDISPO_MS = sec_convert($Sdispo_avg, $TIME_M_agentperfdetail);
		testEcho($TIME_H_agentperfdetail, '$TIME_H_agentperfdetail');
		$USERtotDEAD_MS = sec_convert($Sdead_sec, $TIME_H_agentperfdetail);
		# the dead percentage
		$pfUSERtotDEAD_MS_pct = sprintf("%0.2f", MathZDC(100 * $Sdead_sec, $Stime));
		$USERavgDEAD_MS = sec_convert($Sdead_avg, $TIME_M_agentperfdetail);
	...
	while ($n < $j) {
		...
		if ($status_found < 1) {
			...
			if ($show_percentages) {
				...
				testEcho($Scalls, '$Scalls____');
				testEcho($TOTcalls, '$TOTcalls___');
				$SstatusTXT_pct = sprintf("%8s", sprintf("%0.2f", MathZDC(100 * $Scalls, $TOTcalls)));

				testEcho($SstatusTXT_pct, '$SstatusTXT_pct');
	...
	$graph_id++;
	$graph_array = array(
		"APD_CALLSdata|1|CALLS|integer|",
		"APD_TIMEdata|2|TIME|time|",
		"APD_PAUSEdata|3|PAUSE|time|",
		"APD_PAUSEdata|4|PAUSE AVG|time|",
		"APD_WAITdata|5|WAIT|time|",
		"APD_WAITdata|6|WAIT AVG|time|",
		"APD_TALKdata|7|TALK|time|",
		"APD_TALKdata|8|TALK AVG|time|",
		"APD_DISPOdata|9|DISPO|time|",
		"APD_DISPOdata|10|DISPO AVG|time|",
		"APD_DEADdata|11|DEAD|time|",
		"APD_DEADdata|12|DEAD AVG|time|",
		"APD_CUSTdata|13|CUST|time|",
		"APD_CUSTdata|14|CUST AVG|time|"
	);
	...
	$graph_id++;
	$graph_array = array(
		"APD_TOTALdata|1|TOTAL|time|",
		"APD_NONPAUSEdata|2|NONPAUSE|time|",
		"APD_PAUSEdata|3|PAUSE|time|"
	);
