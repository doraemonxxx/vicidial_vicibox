...
if (isset($_GET["available_chat_agents"])) {
	$available_chat_agents = $_GET["available_chat_agents"];
} elseif (isset($_POST["available_chat_agents"])) {
	$available_chat_agents = $_POST["available_chat_agents"];
}
if (isset($_GET["available_chat_groups"])) {
	$available_chat_groups = $_GET["available_chat_groups"];
} elseif (isset($_POST["available_chat_groups"])) {
	$available_chat_groups = $_POST["available_chat_groups"];
}
if (isset($_GET["available_chat_campaigns"])) {
	$available_chat_campaigns = $_GET["available_chat_campaigns"];
} elseif (isset($_POST["available_chat_campaigns"])) {
	$available_chat_campaigns = $_POST["available_chat_campaigns"];
}
...
if (isset($_GET["allow_replies"])) {
	$allow_replies = $_GET["allow_replies"];
} elseif (isset($_POST["allow_replies"])) {
	$allow_replies = $_POST["allow_replies"];
}
if (isset($_GET["end_all_chats"])) {
	$end_all_chats = $_GET["end_all_chats"];
} elseif (isset($_POST["end_all_chats"])) {
	$end_all_chats = $_POST["end_all_chats"];
}
if (isset($_GET["submit_chat"])) {
	$submit_chat = $_GET["submit_chat"];
} elseif (isset($_POST["submit_chat"])) {
	$submit_chat = $_POST["submit_chat"];
}
if (!$allow_replies) {
	$allow_replies = "N";
}
				...
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
					...
					if (ChatResponseText == 0) {
						...
						for (var i = 0; i < ChatText_array.length; i++) {
							if (ChatText_array[i].length > 0) {
								sub_ids.push(ChatText_array[i]);
							}
						}
						// console.log(sub_ids);
...
if (!$manager_chat_id) { # DO NOT ALLOW NEW CHATS WHILE AN OLD ONE IS OPEN!!!
	...
	while ($row = mysqli_fetch_row($rslt)) {
		if (!in_array("$row[0]", $user_array)) {
			$user_array[$row[0]] = "$row[1]";
		}
		if (!in_array("$row[2]", $user_group_array)) {
			$user_group_array["$row[2]"] = "$row[5]";
		}
		if (!in_array("$row[3]", $campaign_id_array)) {
			$campaign_id_array["$row[3]"] = "$row[4]";
		}
	}
	asort($user_array);
	...
	if (count($user_array) == 0) {
		echo "<option value=''>---- " . _QXZ("NO LIVE AGENTS") . " ----</option>";
	}
	...
	if (count($campaign_id_array) == 0) {
		echo "<option value=''>---- " . _QXZ("NO LIVE CAMPAIGNS") . " ----</option>";
	}
	...
	if (count($user_group_array) == 0) {
		echo "<option value=''>---- " . _QXZ("NO LIVE USER GROUPS") . " ----</option>";
	}
