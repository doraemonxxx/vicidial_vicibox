...
if (isset($_GET["campaigns_or_lists_rpt"])) {
	$campaigns_or_lists_rpt = $_GET["campaigns_or_lists_rpt"];
} elseif (isset($_POST["campaigns_or_lists_rpt"])) {
	$campaigns_or_lists_rpt = $_POST["campaigns_or_lists_rpt"];
}
...
testEcho($stmt, '$vicidial_statuses');
...
testEcho($stmt, '$vicidial_campaign_statuses');
...
while ($lists_to_print > $o) {
	if (preg_match("/$lists[$o]\|/i", $list_string)) {
		$MAIN .= "<option selected value=\"$lists[$o]\">$lists[$o] - $list_names[$o]</option>\n";
	} else {
		$MAIN .= "<option value=\"$lists[$o]\">$lists[$o] - $list_names[$o]</option>\n";
	}
	$o++;
}
...
if (strlen($list[0]) < 1) {
	...
	testEcho($list_id_str, '$list_id_str');

	testEcho($list_stmt, '$list_stmt');
	...
	testEcho($listids_to_print, '$listids_to_print');
	...
	while ($i < $listids_to_print) {
		...
		if ($list_row[2] == "Y") {
			$active_txt = _QXZ("ACTIVE");
		} else {
			$active_txt = _QXZ("INACTIVE");
		}
		...
		testEcho($human_answered_statuses, '$human_answered_statuses');

		testEcho($list_id, '$list_id');

		testEcho($stmt, '$vicidial_list');
		...
		if ($SALE_results > 0) {
			...
			if ($SALE_count > $max_calls) {
				$max_calls = $SALE_count;
			}
		...
		testEcho($completed_statuses, '$completed_statuses');
		testEcho($stmt, '$vicidial_list_completed_statuses');
		...
		if ($COMP_results > 0) {
			...
			if ($COMP_count > $max_calls) {
				$max_calls = $COMP_count;
			}
			$COMP_percent = (MathZDC($COMP_count, $TOTALleads) * 100);
		}

		$HA_percent =	sprintf("%6.2f", "$HA_percent");
		while (strlen($HA_percent) > 6) {
			$HA_percent = substr("$HA_percent", 0, -1);
		}
		$SALE_percent =	sprintf("%6.2f", "$SALE_percent");
		while (strlen($SALE_percent) > 6) {
			$SALE_percent = substr("$SALE_percent", 0, -1);
		}
		$DNC_percent =	sprintf("%6.2f", "$DNC_percent");
		while (strlen($DNC_percent) > 6) {
			$DNC_percent = substr("$DNC_percent", 0, -1);
		}
		$CC_percent =	sprintf("%6.2f", "$CC_percent");
		while (strlen($CC_percent) > 6) {
			$CC_percent = substr("$CC_percent", 0, -1);
		}
		$NI_percent =	sprintf("%6.2f", "$NI_percent");
		while (strlen($NI_percent) > 6) {
			$NI_percent = substr("$NI_percent", 0, -1);
		}
		$UW_percent =	sprintf("%6.2f", "$UW_percent");
		while (strlen($UW_percent) > 6) {
			$UW_percent = substr("$UW_percent", 0, -1);
		}
		$SC_percent =	sprintf("%6.2f", "$SC_percent");
		while (strlen($SC_percent) > 6) {
			$SC_percent = substr("$SC_percent", 0, -1);
		}
		$COMP_percent =	sprintf("%6.2f", "$COMP_percent");
		while (strlen($COMP_percent) > 6) {
			$COMP_percent = substr("$COMP_percent", 0, -1);
		}

		$HA_count =	sprintf("%10s", "$HA_count");
		while (strlen($HA_count) > 10) {
			$HA_count = substr("$HA_count", 0, -1);
		}
		$SALE_count =	sprintf("%10s", "$SALE_count");
		while (strlen($SALE_count) > 10) {
			$SALE_count = substr("$SALE_count", 0, -1);
		}
		$DNC_count =	sprintf("%10s", "$DNC_count");
		while (strlen($DNC_count) > 10) {
			$DNC_count = substr("$DNC_count", 0, -1);
		}
		$CC_count =	sprintf("%10s", "$CC_count");
		while (strlen($CC_count) > 10) {
			$CC_count = substr("$CC_count", 0, -1);
		}
		$NI_count =	sprintf("%10s", "$NI_count");
		while (strlen($NI_count) > 10) {
			$NI_count = substr("$NI_count", 0, -1);
		}
		$UW_count =	sprintf("%10s", "$UW_count");
		while (strlen($UW_count) > 10) {
			$UW_count = substr("$UW_count", 0, -1);
		}
		$SC_count =	sprintf("%10s", "$SC_count");
		while (strlen($SC_count) > 10) {
			$SC_count = substr("$SC_count", 0, -1);
		}
		$COMP_count =	sprintf("%10s", "$COMP_count");
		while (strlen($COMP_count) > 10) {
			$COMP_count = substr("$COMP_count", 0, -1);
		}
	...
	$total_HA_percent =	sprintf("%6.2f", (MathZDC($category_totals["HA"], $totalTOTALleads) * 100));
	while (strlen($total_HA_percent) > 6) {
		$total_HA_percent = substr("$total_HA_percent", 0, -1);
	}
	$total_SALE_percent =	sprintf("%6.2f", (MathZDC($category_totals["SALE"], $totalTOTALleads) * 100));
	while (strlen($total_SALE_percent) > 6) {
		$total_SALE_percent = substr("$total_SALE_percent", 0, -1);
	}
	$total_DNC_percent =	sprintf("%6.2f", (MathZDC($category_totals["DNC"], $totalTOTALleads) * 100));
	while (strlen($total_DNC_percent) > 6) {
		$total_DNC_percent = substr("$total_DNC_percent", 0, -1);
	}
	$total_CC_percent =	sprintf("%6.2f", (MathZDC($category_totals["CC"], $totalTOTALleads) * 100));
	...
	testEcho($totalTOTALleads, '$totalTOTALleads');
	...
	$total_CC_count =	sprintf("%10s", $category_totals["CC"]);
	while (strlen($total_CC_count) > 10) {
		$total_CC_count = substr("$total_CC_count", 0, -1);
	}
	$total_NI_count =	sprintf("%10s", $category_totals["NI"]);
	while (strlen($total_NI_count) > 10) {
		$total_NI_count = substr("$total_NI_count", 0, -1);
	}
	$total_UW_count =	sprintf("%10s", $category_totals["UW"]);
	while (strlen($total_UW_count) > 10) {
		$total_UW_count = substr("$total_UW_count", 0, -1);
	}
	$total_SC_count =	sprintf("%10s", $category_totals["SC"]);
	while (strlen($total_SC_count) > 10) {
		$total_SC_count = substr("$total_SC_count", 0, -1);
	}
	$total_COMP_count =	sprintf("%10s", $category_totals["COMP"]);
	while (strlen($total_COMP_count) > 10) {
		$total_COMP_count = substr("$total_COMP_count", 0, -1);
	}

	testEcho($category_totals, '$category_totals');

	// testEcho($category_totals, '$category_totals');
	testEcho($total_COMP_count, '$total_COMP_count');
	...
	$totalOUToutput .= "\n";
	$totalOUToutput .= "\n";
	$totalOUToutput .= "---------- " . _QXZ("TOTAL STATUS FLAGS SUMMARY 1") . ":    (" . _QXZ("and % of leads in selected lists") . ")     <a href=\"$PHP_SELF?DB=$DB$listQS&SUBMIT=$SUBMIT&file_download=ALL\">" . _QXZ("DOWNLOAD FULL REPORT") . "</a>\n";
	$totalOUToutput .= "+------------------+------------+----------+\n";
