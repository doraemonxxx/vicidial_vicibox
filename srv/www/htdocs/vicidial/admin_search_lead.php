...
if (isset($_GET["log_phone"])) {
	$log_phone = $_GET["log_phone"];
} elseif (isset($_POST["log_phone"])) {
	$log_phone = $_POST["log_phone"];
}
if (isset($_GET["log_lead_id"])) {
	$log_lead_id = $_GET["log_lead_id"];
} elseif (isset($_POST["log_lead_id"])) {
	$log_lead_id = $_POST["log_lead_id"];
}
if (isset($_GET["log_phone_archive"])) {
	$log_phone_archive = $_GET["log_phone_archive"];
} elseif (isset($_POST["log_phone_archive"])) {
	$log_phone_archive = $_POST["log_phone_archive"];
}
if (isset($_GET["log_lead_id_archive"])) {
	$log_lead_id_archive = $_GET["log_lead_id_archive"];
} elseif (isset($_POST["log_lead_id_archive"])) {
	$log_lead_id_archive = $_POST["log_lead_id_archive"];
}
...
if (isset($_GET["alt_phone_search"])) {
	$alt_phone_search = $_GET["alt_phone_search"];
} elseif (isset($_POST["alt_phone_search"])) {
	$alt_phone_search = $_POST["alt_phone_search"];
}
		...
		if ((!$vendor_id) and (!$phone)  and (!$lead_id) and (!$log_phone)  and (!$log_lead_id) and (!$log_phone_archive)  and (!$log_lead_id_archive) and ((strlen($status) < 1) and (strlen($list_id) < 1) and (strlen($user) < 1) and (strlen($owner) < 1)) and ((strlen($first_name) < 1) and (strlen($last_name) < 1) and (strlen($email) < 1))) {
			...
			if ((strlen($log_lead_id) > 0) or (strlen($log_phone) > 0)) {
				...
				if (($results_to_print < 1) and ($results_to_printX < 1)) {
					...
					echo "<BR><b>" . _QXZ("OUTBOUND LOG RESULTS1") . ": $results_to_print</b><BR>\n";
			...
			if ((strlen($log_lead_id_archive) > 0) or (strlen($log_phone_archive) > 0)) {
				...
				if (($results_to_print < 1) and ($results_to_printX < 1)) {
					...
					echo "<BR><b>" . _QXZ("OUTBOUND LOG RESULTS2") . ": $results_to_print</b><BR>\n";
					echo "<TABLE BGCOLOR=WHITE CELLPADDING=1 CELLSPACING=0 WIDTH=770>\n";
					echo "<TR BGCOLOR=BLACK>\n";
					echo "<TD ALIGN=LEFT VALIGN=TOP><FONT FACE=\"ARIAL,HELVETICA\" COLOR=WHITE><B>#</B></FONT></TD>\n";
					echo "<TD ALIGN=CENTER VALIGN=TOP><FONT FACE=\"ARIAL,HELVETICA\" COLOR=WHITE><B>" . _QXZ("LEAD ID") . "</B> &nbsp;</FONT></TD>\n";
				...
				if ($phone) {
					...
					if ($lead_id) {
						...
						if ((strlen($status) > 0) or (strlen($list_id) > 0) or (strlen($user) > 0) or (strlen($owner) > 0)) {
							...
							$called_countSQL = '';
							if (strlen($status) > 0) {
								$statusSQL = "status='" . mysqli_real_escape_string($link, $status) . "'";
								$SQLctA++;
							}
							if (strlen($list_id) > 0) {
								if ($SQLctA > 0) {
								...
								$list_idSQL = "$andA list_id='" . mysqli_real_escape_string($link, $list_id) . "'";
								$SQLctB++;
							}
							if (strlen($user) > 0) {
								if (($SQLctA > 0) or ($SQLctB > 0)) {
									$andB = 'and';
								}
								$userSQL = "$andB user='" . mysqli_real_escape_string($link, $user) . "'";
								$SQLctC++;
							}
							if (strlen($owner) > 0) {
								if (($SQLctA > 0) or ($SQLctB > 0) or ($SQLctC > 0)) {
									$andC = 'and';
								}
							...
							if ((strlen($first_name) > 0) or (strlen($last_name) > 0)) {
								...
								if (strlen($first_name) > 0) {
									$first_nameSQL = "first_name='" . mysqli_real_escape_string($link, $first_name) . "'";
									$SQLctA++;
								}
